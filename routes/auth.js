...
async function authorize(user) {
    ...
    await TokenService.saveTokens(userDTO.id, tokens.refresh_token); // записываем токен пользователю
...
router.post('/login', loginValidators, async (req, res) => {
    ...
    if (password) {
        ...
        if (isEqual) {
            ...
            res.cookie('refresh_token', userDTO, { maxAge: 30 * 24 * 60 * 60 * 1000, httpOnly: true });
